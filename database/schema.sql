...
CREATE TABLE IF NOT EXISTS conversations (
  ...
  state TEXT NOT NULL DEFAULT 'open', -- Estado da conversa: open, waiting, archived
  last_message TEXT, -- Mantido para compatibilidade (texto simples)
  last_message_id TEXT, -- ID da última mensagem (para construir objeto lastMessage)
  last_message_at TIMESTAMP,
  unread_count INTEGER DEFAULT 0, -- Contador de mensagens não lidas
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW(),
  CONSTRAINT conversations_state_check CHECK (state IN ('open', 'waiting', 'archived'))
);
